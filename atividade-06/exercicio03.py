# Implemente uma função que crie uma cópia de uma árvore binária. Esta função deve obedecer ao
# protótipo: def copia(arv)
...
def copia(root):
    ...
    novo_no = No(root.chave)
    novo_no.esquerda = copia(root.esquerda)
    novo_no.direita = copia(root.direita)
    return novo_no
...
copia_raiz = copia(arvore.raiz)
print("\nEm ordem Arove copia:")
em_ordem(copia_raiz)